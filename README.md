...
```
브라우저 ──쿠키── React Router 서버 ──Bearer── 백엔드
```

브라우저는 백엔드를 직접 부르지 않는다. **그래서 이 앱에는 CORS 가 없다.**
...
**커밋은 의미 단위로 잘게 쪼갠다.** 하나를 되돌려도 나머지가 성립해야 하고, **각 커밋이 그 자체로 `tsc`·`build` 를 통과해야 한다.** 생성물(`schema.d.ts`)과 문서는 코드와 섞지 않고 따로 커밋한다.
...
**PR·이슈 본문에 실제 서버 주소나 키를 쓰지 않는다.** 저장소 접근 권한이 있는 사람 전부가 보고 알림 메일로도 나간다. 터미널 출력을 붙일 때 특히 새기 쉬우므로 `${API_BASE_URL}` 같은 자리표시자로 바꾼다.
...
- **화면 상태를 따로 들고 있지 않는다.** 데이터에서 유도한다. 따로 들면 실제와 어긋난 화면을 그린다.
- 고른 값은 주소에 둔다(`?photo=` · `?product=`). 새로고침해도 남고 링크로 공유된다.
- 주소의 id 는 **양수만** 받는다. `Number(null)` 은 `0` 이고 `Number.isInteger(0)` 은 참이라, 파라미터가 없을 때 0 이라는 멀쩡해 보이는 id 가 만들어진다.

### 실패 처리

- **화면 전체를 죽이지 않는다.** 헤더는 남기고 그 자리에만 실패를 알린다. 다른 화면으로 나갈 길을 막지 않는다.
- **막다른 길을 만들지 않는다.** 실패 화면에는 항상 다음 행동(다시 시도·목록으로)을 둔다.
- **백엔드 에러 메시지를 그대로 띄우지 않는다.** `IMAGE_GENERATION_ERROR` 같은 내부 사정이 사용자에게 샌다. 시안이 정한 문구를 쓰고, 원인은 터미널 로그에 남긴다.
- 되돌릴 수 없는 동작(사진 삭제·기준 이미지 재생성)은 **누르기 전에 무엇이 사라지는지 알리고 한 번 더 묻는다.**
